...
$=(2(63)+1)$
$=(126+1)$
$=127 \Longrightarrow \boxed{\textbf{(C)}\ 127}$.
...
Adding the areas together, we have $60 ft^{2} + 18 ft^{2} = 78 ft^{2} \Longrightarrow \boxed{\textbf{(B)}\ 78}$.
~JH. L
...
See for yourself. And by looking into fractions, we immediately see that $\frac{1}{3}$ and $1$ would fit the rule. $\boxed{\textbf{(C)} 4}.$
...
If that someone is Lewis the statement becomes: "if Lewis got all the multiple choice questions right, then he would receive an A on the exam."
...
Let $j$ represent how far Jerry walked, and $s$ represent how far Silvia walked. Since the field is a square, and Jerry walked two sides of it, while Silvia walked the diagonal, we can simply define the side of the square field to be one, and find the distances they walked. Since Jerry walked two sides, 
$j = 2$
Since Silvia walked the diagonal, she walked the hypotenuse of a $45$, $45$, $90$ triangle with leg length $1$. Thus,
$s = \sqrt{2} = 1.414...$
We can then take 
$\frac{j-s}{j} \approx \frac{2 - 1.4}{2}=0.3 \implies \boxed{\textbf{(A)}\ 30\%}$

**Solution 2.**

We can assume that Jerry's path is $100$ units in length. By that logic, Silvia's path is $\frac{100\cdot\sqrt{2}}{2}=50\sqrt{2}$. We can assume that $\sqrt{2}\approx1.4$, so we find that Jerry's path is $70$ units in length. We can now subtract the two paths, and find out that the answer is $\boxed{\textbf{(A)}\ 30\%}$.
~demonhunter
...
## Problem 8
*Tags: arithmetic-series, complementary-counting*
...
$\frac{10(20+29)}{2}\implies 5(49)\implies 245.$
Therefore, the answer is $\boxed{\textbf{(B)}\ 245}$
...
A pattern starts to emerge as the function is continued. The repeating pattern is $0,1,1,2,0,2,2,1\ldots$ The problem asks for the sum of eight consecutive terms in the sequence. Because there are eight numbers in the repeating pattern, we just need to find the sum of the numbers in the sequence, which is $\boxed{\textbf{(D)}\ 9}$
...
Adding, we get $\frac{2300}{99}$, which is closest to $23$ which is $\boxed{\textbf{(D)}\ 23\%}$.
...
## Problem 15
*Tags: arithmetic-series, casework, congruent-triangles, geometric-probability*
...
Suppose a point $(x,y)$ lies in the $xy$-plane. Let $x$ be Chloe's number and $y$ be Laurent's number. Then obviously we want $y>x$, which basically gives us a region above a line. We know that Chloe's number is in the interval $[0,2017]$ and Laurent's number is in the interval $[0,4034]$, so we can create a rectangle in the plane, whose length is $2017$ and whose width is $4034$. Drawing it out and dividing into 4 congruent triangles, we see that Laurent's winning area is 3 triangles and Chloe's  is 1 triangle. $\boxed{\textbf{(C)}\ \frac{3}{4}}$.
...
## Problem 18
*Tags: casework, factoring, geometric-series*
...
Let $P$ be the probability Amelia wins. Note that $P = \text{chance she wins on her first turn} + \text{chance she gets to her second turn}\cdot \frac{1}{3} + \text{chance she gets to her third turn}\cdot \frac{1}{3} \cdots$ This can be represented as an infinite geometric series: $$P=\frac{\frac{1}{3}}{1-\frac{2}{3}\cdot \frac{3}{5}} = \frac{\frac{1}{3}}{1-\frac{2}{5}} = \frac{\frac{1}{3}}{\frac{3}{5}} = \frac{1}{3}\cdot \frac{5}{3} = \frac{5}{9}.$$ 
Therefore, $P = \frac{5}{9}$, so the answer is $9-5 = \boxed{\textbf{(D)}\ 4}.$

Solution by ktong

~minor LaTeX edit by virjoy2001
~quicky grammar edit by NSAoPS
...
$$\dfrac{1}{3}+\left(\dfrac{2}{3}\cdot\dfrac{3}{5}\right)\cdot\dfrac{1}{3}+\left(\dfrac{2}{3}\cdot\dfrac{3}{5}\right)^2\cdot\dfrac{1}{3}+\cdots.$$
Factoring out $\dfrac13$ we get a geometric series:
...
Since B and C can't sit next to A, that must mean either D or E sits next to A. 
After we pick either D or E, then either B or C must sit next to D/E. 
Then, we can arrange the two remaining people in two ways. 
Since there are two different edge seats that A can sit in, there are a total of $2 \cdot 2 \cdot 2 \cdot 2 = 16$.
...
So in total there are $12+16=28$ ways. Our answer is $\boxed{\textbf{(C)}\ 28}$.
<br>
Minor $\LaTeX$ edits by fasterthanlight :)
...
We add up the cases where one of these are true:
$48\cdot3=144$
...
So our answer is $\boxed{\textbf{(D)}\ 1239}$.
...
We note quickly from the given value of $S(n)$ that the form of n is likely going to be of the form $999999999...X9...9999$  
...
Plugging in answer choices for $A$ we quickly see that 
$\boxed{\textbf{(D) } 1239}.$ is our answer
...
~ [https://youtu.be/dQw4w9WgXcQ Shreyansh]
...
&=x^4+(a+p)x^3+(1+ap)x^2+(10+p)x+10p.\end{align*}$$
...
**Solution 9 (Vieta System).**

Let $p,q,$ and $r$ be the roots of $g(x),$ and $s$ be the additional root of $f(x).$ By Vieta's formulas, it follows that
$$\begin{cases}p+q+r=-a &(1)\\pq+pr+qr=1 &(2)\\pqr=-10 &(3)\\ p+q+r+s=-1 &(4)\\ pqrs=c &(5)\\pq+pr+ps+qr+qs+rs=b &(6)\\
pqr+pqs+prs+qrs=-100 &(7)\end{cases}$$
Looking at $(7),$ we have that
$$pqr+s(pq+pr+qr)=-10+s=-100\implies s=-90$$
then looking at $(5)$ we have that 
$$c=-10\cdot-90=900$$
looking at $(4)$ yields
$$-1-s=-a\implies a=s+1=-89$$
and looking at $(6)$ yields
$$1+s(p+q+r)=1-as=1-90\cdot89=-8009=b.$$
Thus,
$$f(1)=1+1-8009+100+900=\boxed{\textbf{(C) }-7007}$$
~shockfront99
...
## Problem 25
*Tags: casework, divisibility-rules*
