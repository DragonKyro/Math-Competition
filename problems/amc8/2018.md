...
We can compute $\frac{289}{20}$ and round our answer to get $\boxed{\textbf{(A) }14}$.
...
**Solution 2 (use this when your brain no longer works).**

$\frac{2}{1} \cdot \frac{3}{2} \cdot \frac{4}{3} \cdot \frac{5}{4} \cdot \frac{6}{5} \cdot \frac{7}{6} = \boxed{\textbf{(D)}\ 7}$.
~themathkidwoohooooo
...
Rearranging the terms, we get $1+(3-2)+(5-4)+(6-5)...(2017-2016)+(2019-2018)$, and our answer is $1+1009=\boxed{\textbf{(E) }1010}$.
...
_(Asymptote diagram omitted — see AoPS for image)_
What was the mean number of days of exercise last week, rounded to the nearest hundredth, reported by the students in John Pork's class? (Real problem uses Mr. Garcia, not John Pork)
...
~small edit by i_am_not_suk_at_math

~minor edit by Flexo3168
...
We can ignore other students, and treat Abby and Bridget as indistinguishable (since we only care about adjacency, not their order). Thus, the total number of ways is $n(S) = _{6}C_{2} = 15$ .
In one row, they can be adjacent 2 ways:  $2 \cdot 2 rows = 4$. 
In one column, they can only be adjacent 1 way: $1 \cdot 3 cols = 3$. 
Add these cases $4+3=7$, and therefore, P(Abby and Bridget sitting adjacent) is $\boxed{\textbf{(C) }\frac{7}{15}}$.
...
To get the area of the small circles, we can get the equation $\frac{1}{2}= \pi r^2$. Solving for $r$, we get $r = \sqrt{\frac{1}{2\pi}}$. Then, we can get the radius of the big circle by doubling the small circle's radius, and that gives
...
Multiplying all these possibilities(2!,4!,5!) gives the total number of arrangements which is $\boxed{\textbf{(C) }5760}.$ ways to arrange the nine books while keeping the Arabic and Spanish books together.
-TORPEDOROCKS
...
## Problem 18
*Tags: divisibility-rules, factoring, prime-factorization*
...
~small fix by i_am_not_suk_at_math
...
Together, we find that the answer is $2+2+2+2=\boxed{\textbf{(C) } 8}$.
...
**Solution 8 (Very quick, uses formula).**
...
**Solution 9 (Modified version of Sol 1).**

The top block must be positive so below it two box must be of same sign. 
Now if we arrange the bottom row as following pattern such that according to that the row above bottom will be changed and so on , so that the 3rd two blocks are of same sign hence the top block will be positive.(Draw separate diagram and do yourself to understand )

+−−+,  −++−, −−−−, ++++, −+−+, +−+−, ++−−, −−++. 

There are 8 patterns .
Hence $\boxed{\textbf{(C) } 8}$.
...
## Problem 24
*Tags: brahmaguptas-formula, coordinate-bash, law-of-cosines, pythagorean-theorem*
...
Note that $EJCI$ is a rhombus by symmetry.
Let the side length of the cube be $s$. By the Pythagorean theorem, $EC= s\sqrt 3$ and $JI= s\sqrt 2$. Since the area of a rhombus is half the product of its diagonals, the area of the cross section is $\frac{s^2\sqrt 6}{2}$. This gives $R = \frac{\sqrt 6}2$. Thus, $R^2 = \boxed{\textbf{(C) } \frac{3}{2}}$.
...
Brahmagupta's formula states given a quadrilateral $ABCD$ with sides $a,b,c,d$ then $$[ABCD]=\sqrt{(s-a)(s-b)(s-c)(s-d)-abcd\cos^2\left({\frac{B+D}{2}}\right)}$$where $s=\frac{a+b+c+d}2$. Using this formula, we find that$$[EJCI]=\sqrt{(2\sqrt5-\sqrt5)(2\sqrt5-\sqrt5)(2\sqrt5-\sqrt5)(2\sqrt5-\sqrt5)-\sqrt 5^4(\tfrac 15)^2}=2\sqrt{6}.$$ 
...
Note: Of course this is a solution made by Franz Liszt.
...
\begin{align*}
\text{Area} &= \frac{4\sqrt{3} \cdot 4\sqrt{2}}{2} \\
&= 8\sqrt{6}.
\end{align*}
...
(You can do this, but it will still take a few minutes. Try to do something else)

==Solution 2 (If you do not notice that $2^{18}=(2^6)^3=64^3$)==
...
We can immediately evaluate $2^8+1$ which gives us $257$ and the smallest perfect cube greater than that is $343 = 7^3$, and you also immediately notice that $2^{18} = (2^6)^3 = 64^3$. Now, all we have to do is count the cubes, which equals:
\begin{align}
...
**Solution 4(Bascially the same as solution 3).**
