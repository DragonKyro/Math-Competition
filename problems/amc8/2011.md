...
The taxi fare in Gotham City is <nowiki>$2.40</nowiki> for the first $\frac12$ mile and additional mileage charged at the rate <nowiki>$0.20</nowiki> for each additional 0.1 mile. You plan to give the driver a <nowiki>$2</nowiki> tip. How many miles can you ride for <nowiki>$10</nowiki>?
...
$$\begin{align*} 10 &= 2.4 + 0.2 \times \frac{x-0.5}{0.1} + 2\\
5.6 &= 2(x-0.5)\\
2.8 &= x-0.5\\
x &= \boxed{\textbf{(C)}\ 3.3}\end{align*}$$
...
Three people can be seated opposite of Angie (Bridget, Carlos, or Diego). Only one of these is Carlos. Thus, the answer is $\boxed{\textbf{(B) }\frac{1}{3}}$.
- @scthecool
...
<b> Assume </b> the triangles are right. Then, the 25-25-30 has hypotenuse 30, whereas 25-25-40 has hypotenuse 40. Because the formula for a triangles' area is $ (bh)/2 $, the hypotenuse does not matter. So, since the base and height are the same, the area will also be the same meaning $$A = B \hspace{0.15in} \Longrightarrow \hspace{0.15in} \boxed{\textbf{(C)}}$$
...
===Sidenote===
This questions expects you to actually HAVE a 5 in it.
...
But, $9999$ is clearly divisible by $3$,  
...
~RandomMathGuy500
Comment from WrenMath: This is literally the same thing.
Comment from dnathan12: Bruh stop hating, honestly this solution is easier.
...
The area of the smaller square is one half of the product of its diagonals. Note that the distance from a corner of the smaller square to the center is equivalent to the circle's radius so the diagonal is equal to the diameter: $2 \cdot 
2 \cdot \frac{1}{2}=2.$
...
For the ratio of the circle's shaded area to the area between the squares to be $1,$ they would have to be approximately the same size. For any ratio larger than that, the circle's shaded area must be greater. However, we can clearly see that the circle's shaded area is part of the area between the squares, and is approximately $\boxed{\textbf{(A)}\ \frac12}$. The ratio must be less than 1, because the area inside the circle is less than the total shaded area, and the only answer choice that fits that is ${\frac12}$.
Note that this solution is not rigorous, because we still should show that the ratio is less than $\frac{3}{4}$.

~Slightly edited by rwpiano
...
So $32/64$ is $1/2$, or $\boxed{\textbf{(A)}\ \frac12}$.
