...
$\textbf{(A)}\ \frac{12-5\sqrt3}{72} \qquad
\textbf{(B)}\ \frac{12-5\sqrt3}{36} \qquad
...
Two circles intersect if the distance between their centers is less than the sum of their radii. In this problem, $A$ and $B$ intersect if 
$\sqrt{(A_X-B_X)^2+(A_Y-B_Y)^2}\leq2 \Rightarrow (A_X-B_X)^2+1\leq4 \Rightarrow (A_X-B_X)\leq \sqrt{3}.$
...
## Problem 23
*Tags: base-representations, casework, floor-function, logarithms, parity*
...
## Problem 24
*Tags: arithmetic-series, quadratic-formula, recursion*
...
$$(y \sqrt{3})^{2} = (y+\frac{2}{3}).$$
Solving using the quadratic formula gives 
$$y = \frac{-(-1) \pm \sqrt{1^{2}-4(3)(-\frac{2}{3})}}{6} = \frac{1 \pm \sqrt{1^{2}+8}}{6} = \frac{2}{3}.$$
Then, $2y = \frac{4}{3},$ so $A_{2} = (2,0).$
...
$$x = \frac{1 \pm \sqrt{1+12a_{n}}}{6},$$
but since $x$ must be positive, we have that
$$x = \frac{1 + \sqrt{1+12a_{n}}}{6},$$
and so 
$$a_{n+1} = a_{n}+2x = a_{n}+\frac{1 + \sqrt{1+12a_{n}}}{3}.$$
Computing a few terms of $a_{n}$ using this method gives $a_{3} = 4,$ $a_{4} = \frac{20}{3},$ and $a_{5} = 10$. 
...
$$1+12a_{n} = (2n+1)^{2}.$$
We will prove this with induction. The base case, $n=2$, we have already verified.
...
$$a_{n} = \frac{(2n+1)^{2}-1}{12}.$$
Therefore, we have proved our claim. Now, we have that $|A_{0}A_{n}|=a_{n},$ so we just need the least integer $n$ so that 
$$a_{n} > 100,$$
or
$$(2n+1)^{2} > 1201.$$
Then, we see that $35^{2} = 1225$ is the smallest odd square larger than $1201.$ Therefore, we have $2n+1=35$, so $n = \boxed{17}.$
...
## Problem 25
*Tags: angle-chasing, factoring, herons-formula, pythagorean-theorem*
