...
## Problem 9
*Tags: chinese-remainder-theorem, floor-function, logarithms, modular-arithmetic*
...
Let $n = 2^2 \cdot 5 \cdot x$ for some integer $x$, then we know that 
\begin{align*}
...
$2^4 \cdot 5^2 \cdot 2^{2\alpha} \cdot 5^{2\beta} = 2^{4 + 2\alpha} \cdot 5^{2 + 2\beta} = m_0^3$, therefore we know 
\begin{align*}
...
2 + 2\beta \equiv 0 \pmod{3}\\
\end{align*}
And using [[Chinese Remainder Theorem]] to solve this, we get $\alpha = 4$ and $\beta = 5$, and plugging that back in yields $n = 2^6 \cdot 5^6$.
...
has magnitude less than or equal to $ 1$. (Can you show how?) Hence, the probability is $ \boxed{\text{(C)} \frac {1}{4}}$.
...
$$f(\theta)=\frac{\theta}{2\pi}\cdot2\pi=\theta$$ 
...
$$\frac{1}{\pi}\int_0^{\pi}\theta\;d\theta=\frac{\pi}{2}$$
The probability that the third jump will land on this arc length is just the arc length divided by the circumference, or $\frac{\frac{\pi}{2}}{2\pi}=\frac{1}{4} \implies \boxed{C}$
...
$P(Q(x)) = x^4 + 76x^3 + (1552 + 2F)x^2 + (76F + 4104)x + (F^2 + 108F + C)$
$Q(P(x)) = x^4 + 216x^3 + (11702 + 2C)x^2 + (216C + 4104)x + (C^2 + 38C + F)$
...
This simplifies into $$\frac{-x^3+3x^2+x-1}{(x)(x+1)(x-1)}\ge0,$$ so $$-x^3+3x^2+x-1\ge0.$$
Each interval is $(-1, a), (0, b), (1, c)$, where $a$, $b$, and $c$ are the roots of $-x^3+3x^2+x-1=0$
so the total length is $a+b+c$, which is the sum of the roots, or $\boxed3$.
