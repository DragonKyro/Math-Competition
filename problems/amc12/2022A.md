...
Five rectangles, $A$, $B$, $C$, $D$, and $E$, are arranged in a square as shown below. These rectangles have dimensions $1\times6$, $2\times 4$, $5\times6$, $2\times7$, and $2\times3$, respectively. (The figure is not drawn to scale.) Which of the five rectangles is the shaded one in the middle?
...
## Problem 5
*Tags: arithmetic-series, casework, picks-theorem, stars-and-bars*

The <em>taxicab distance</em> between points $(x_1, y_1)$ and $(x_2, y_2)$ in the coordinate plane is given by $$|x_1 - x_2| + |y_1 - y_2|.$$
...
$$\begin{align*}
1 + 3 + 5 + \cdots + 41 + 39 + 37 + \cdots + 1 &= (1 + 3 + 5 + \cdots + 39) + (1 + 3 + 5 + \cdots + 41) \\
& = 20^2 + 21^2 \\
& = 29^2 \\
&= \boxed{\textbf{(C)} \, 841}.
\end{align*}$$
...
It is clear what the pattern is. Setting the answer as $P,$ we have $$P = \sqrt[3]{10P},$$ from which $P = \boxed{\sqrt{10}}.$
...
$$
Subbing in for $2d$ and using log rules,
$$
...
Without loss of generality, let tetrahedron $ABCD$ lie in three-dimensional space with vertices 
$A(-1, 1, 1)$, $B(1, 1, -1)$, $C(1, -1, 1)$, and $D(-1, -1, -1)$.  
Let point $M$ be located at $(0, 1, 0)$.
...
Finally: $$\cos(\angle CMD) = \frac{2}{6} = \boxed{\textbf{(B) } \frac13}.$$
~TylerTrikowsky
...
$$= 2((\log 5)^{2}-(\log 5)(2\log 2 + \log 5) + (2\log 2 + \log 5)^{2})$$
Let x = $\log 5$ and y = $\log 2$, so $x+y=1$
...
$$0.7^3 + 1.3^3 + .9\cdot(-0.6) = \boxed{2}$$
~kxiang

**Solution 4(log bash).**

Using log properties, we combine the terms to make our expression equal to $\log {( (5^{\log^2{5}}) \cdot (20^{\log^2{20}}) \cdot 8 ^ {\log{\frac{1}{4}}} ) }$. 
...
Thus, $n (n+1) = 2 k^2$. Rearranging, we get $(2n+1)^2-2(2k)^2=1$, a Pell equation (see https://artofproblemsolving.com/wiki/index.php/Pell_equation
). So $\frac{2n+1}{2k}$ must be a truncation of the continued fraction for $\sqrt{2}$:
...
$$n_6 = ( \sqrt{2n_5} + \sqrt{n_5 + 1})^2 = ( 41\sqrt{2} + \sqrt{1682})^2 =  ((41 + 29)\sqrt{2})^2 = 9800,... $$
Therefore, $t_{n_4} = t_{288} = \frac{288\cdot289}{2} = 41616 \implies 4+1+6+1+6=\boxed{\textbf{(D) 18}}$
...
## Problem 17
*Tags: angle-addition-formulas, double-angle-identities, quadratic-formula, sum-to-product*
...
## Problem 21
*Tags: complex-numbers, modular-arithmetic, prime-factorization, quadratic-formula, roots-of-unity*
...
This automatically eliminates choices $A$ , $C$ and $D$ since they do not form a factor of $1$ at $x = -1$.
...
This eliminates choice $B$, leaving us with answer $\boxed{E}$.


Solution $5.2$(Mods):
Plug in $2$ for $x$ at choice $B$. $2^2+2*2+1=7$.
The cycle of mods for $2$ and $7$ is $2$, $4$, $1$. Both $2022$ and $1011$ are divisible by $3$, so we have $1$ + $1$ + $1$ = $3$. $3$ is not divisible by $7$, so we eliminate choice $B$. This leaves us with $\boxed{(E) x^{6}+x^{3}+1}$.
...
## Problem 22
*Tags: am-gm-inequality, complex-numbers, vietas-formulas*
...
## Problem 25
*Tags: casework, diophantine-equations, pythagorean-theorem, simons-favorite-factoring*
...
uv = 2 r^2 ,
$$
we observe that the R.H.S. is a not a perfect square. Thus, the number of positive $(u, v)$ is equal to the number of positive divisors of $2 r^2$.
